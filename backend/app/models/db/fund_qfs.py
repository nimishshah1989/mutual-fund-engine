...
13 metrics x 4 horizons -> min-max normalised -> time-weighted -> QFS (0-100).
...
class FundQFS(Base):
    ...
    # Weighted Fund Score = 1*score_1y + 2*score_3y + 3*score_5y + 4*score_10y
    ...
    data_completeness_pct: Mapped[Optional[float]] = mapped_column(
        Numeric(5, 2), comment="% of 52 possible data points available"
    )
