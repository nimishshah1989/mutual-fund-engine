...
class MetricHorizonDetail(BaseModel):
    ...
    raw: Optional[DecimalFloat] = Field(None, description="Raw metric value from data source")
    normalised: Optional[DecimalFloat] = Field(None, description="Min-max normalised score (0-100)")
...
class ScoreDetail(BaseModel):
    ...
    mstar_id: str
    computed_date: date
    qfs: DecimalFloat = Field(..., description="Final Quantitative Fund Score (0-100)")
    wfs_raw: Optional[DecimalFloat] = Field(None, description="Weighted Fund Score before final normalization")
    score_1y: Optional[DecimalFloat] = Field(None, description="Average normalised score for 1-year horizon")
    score_3y: Optional[DecimalFloat] = Field(None, description="Average normalised score for 3-year horizon")
    score_5y: Optional[DecimalFloat] = Field(None, description="Average normalised score for 5-year horizon")
    score_10y: Optional[DecimalFloat] = Field(None, description="Average normalised score for 10-year horizon")
    data_completeness_pct: Optional[DecimalFloat] = Field(
...
class SectorContributionDetail(BaseModel):
    ...
    exposure_pct: DecimalFloat = Field(..., description="Fund's allocation to this sector (%)")
    benchmark_weight_pct: DecimalFloat = Field(Decimal("0"), description="Benchmark allocation to this sector (%)")
    active_weight: DecimalFloat = Field(Decimal("0"), description="Fund exposure - benchmark weight")
    signal: str = Field(..., description="FM signal for this sector")
    signal_weight: DecimalFloat = Field(..., description="Numeric weight of the signal")
    confidence: str = Field(..., description="FM confidence level")
    confidence_multiplier: DecimalFloat = Field(..., description="Numeric multiplier for confidence")
    contribution: DecimalFloat = Field(..., description="Sector contribution to raw FMS")
...
class FSASDetail(BaseModel):
    ...
    raw_fsas: Optional[DecimalFloat] = Field(None, description="Raw FSAS before normalization")
    fsas: DecimalFloat = Field(..., description="Normalised FSAS (0-100)")
    ...
    avoid_exposure_pct: DecimalFloat = Field(Decimal("0"), description="Total % in AVOID sectors")
...
class RecommendationDetail(BaseModel):
    ...
    mstar_id: str
    computed_date: date
    qfs: DecimalFloat = Field(..., description="QFS score (0-100)")
    fsas: Optional[DecimalFloat] = Field(None, description="FM Alignment Score (0-100)")
    qfs_rank: int = Field(..., description="Rank within category (1 = best)")
    category_rank_pct: DecimalFloat = Field(..., description="QFS percentile rank (0-100, 100 = best)")
    ...
    fm_score_percentile: Optional[DecimalFloat] = Field(None, description="FMS percentile in category")
    qfs_percentile: Optional[DecimalFloat] = Field(None, description="QFS percentile in category")
...
class ShortlistItem(BaseModel):
    ...
    fsas: Optional[DecimalFloat] = None
    tier: Optional[str] = None
    action: Optional[str] = None
    avoid_exposure_pct: Optional[DecimalFloat] = None
...
class ScoreOverviewItem(BaseModel):
    ...
    computed_date: date
    qfs: DecimalFloat
    wfs_raw: Optional[DecimalFloat] = None
    score_1y: Optional[DecimalFloat] = None
    score_3y: Optional[DecimalFloat] = None
    score_5y: Optional[DecimalFloat] = None
    score_10y: Optional[DecimalFloat] = None
    data_completeness_pct: Optional[DecimalFloat] = None
    ...
    category_rank_pct: Optional[DecimalFloat] = Field(None, description="Percentile rank in category")
